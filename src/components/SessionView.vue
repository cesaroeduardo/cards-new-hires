<template>
  <div class="max-w-7xl mx-auto py-2 lg:px-8 px-4">
    ...
    <div v-if="!userName" class="flex flex-col items-center gap-5 w-full">
      <h2 class="text-xl font-medium text-black dark:text-white font-mono">Digite seu nome para entrar na sessão</h2>
      <input v-model="userNameInput" placeholder="Seu nome" class="border text-sm p-2 rounded-md min-w-60" />
      <button @click="saveUserName" class="bg-orange-600 text-white px-4 py-2 text-sm rounded-md hover:bg-orange-500 transition font-medium">Entrar na Sessão</button>
      <div v-if="message" class="text-red-500 mt-4">{{ message }}</div>
    </div>

    <!-- Tela da sessão -->
    <div @mousemove="updateMousePosition" v-else class="flex justify-center items-start flex-col lg:flex-col gap-10 relative">
      <div class="flex flex-col md:flex-row gap-12 w-full lg:w-full">
        <!-- Lista de usuários conectados -->
        <ul class="flex min-w-[280px] w-full md:max-w-[320px] flex-col h-auto justify-start rounded-lg p-4 text-black dark:text-white bg-[#1e1e1e05] dark:bg-white/5 border border-[#1e1e1e15] dark:border-white/10 gap-2">
          <div class="flex flex-row items-center w-full px-2">
            <h2 class="text-[10px] w-full text-left font-mono opacity-35 font-medium uppercase tracking-[.2rem]">Participantes</h2>
            <!-- Botão de Compartilhamento -->
            <button @click="shareLink" class="text-black text-[10px] dark:text-white px-2 !w-auto py-2 opacity-40 rounded-lg dark:hover:bg-white/10 transition font-medium flex items-center gap-2">
          ...
          <li v-if="connectedUsers.length > 0" v-for="user in connectedUsers" :key="user.id" class="text-sm font-mono opacity-70 text-left px-2 lowercase">{{ user.name }}</li>
          <p v-else class="text-xs text-left px-2 py-1 dark:text-white text-black font-mono opacity-20">Carregando participantes...</p>
        </ul>

        <!-- Boas vindas e instruções -->
        <div class="flex flex-col items-start gap-4 max-w-[440px]">
          <h3 class="text-xl font-medium text-black dark:text-white font-mono">Boas-vindas à Azion <span class="text-orange-600">{{ userName }}!</span></h3>
          <p class="text-[10px] w-full text-left font-mono opacity-35 text-black dark:text-white font-medium uppercase tracking-[.2rem]">Sobre a dinâmica</p>
          <p class="text-xs flex-col text-left text-pretty dark:text-white text-black font-mono opacity-70 leading-relaxed">
            ...
            <br /><br /><span class="text-orange-500 font-bold text-md">Atenção: Espere o facilitador chamar a sua vez para clicar nas cartas.</span>
          </p>
        </div>
      </div>

      <!-- Container para exibir as cartas -->
      <div class="flex flex-wrap w-full items-start justify-between gap-1 lg:justify-start gap-y-1">
        <Card v-for="(card, index) in cards" :key="index" :cardNumber="card.number" :backTitle="card.backTitle" :backContent="card.backContent" :backImage="card.backImage" :canFlip="!card.flipped" :isFlipped="card.flipped" @flip-card="flipCard(index)" />
      </div>

      <!-- Exibição dos ponteiros dos outros usuários -->
      <div v-for="pointer in validUserPointers" :key="pointer.user_name" class="absolute" :style="{ top: pointer.y - 55 + 'px', left: pointer.x - 95 + 'px' }">
        <span class="bg-orange-600 text-white rounded-full px-2 py-1 text-xs lowercase">{{ pointer.user_name }}</span>
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    async joinSession() {
      try {
        const { data, error } = await supabase.from('sessions').select('id, flipped_cards').eq('session_code', this.sessionCode.trim()).single();
        ...
        const { data: existingUser } = await supabase.from('users').select('id').eq('session_id', this.sessionId).eq('name', this.userName).single();

        if (!existingUser) {
          await supabase.from('users').insert([{ name: this.userName, session_id: this.sessionId }]);
        }
    ...
    async loadConnectedUsers() {
      const { data, error } = await supabase.from('users').select('*').eq('session_id', this.sessionId);
    ...
    updateUserPointer(pointer) {
      const index = this.userPointers.findIndex((p) => p.user_name === pointer.user_name);
      if (index !== -1) {
